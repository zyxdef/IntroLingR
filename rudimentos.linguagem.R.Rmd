...
tipo ou então eles são forçados por regras de coerção.
...
1. o primeiro caractere pode ser uma letra ou o símbolo `"."` (ponto)
1. o segundo caractere (se houver) pode ser uma letra, o símbolo `"."` (ponto) 
ou o símbolo `"_"` (sublinhado)
    - se o primeiro caractere não for `"."` (ponto), o segundo caractere também 
    poderá ser um dígito
1. do terceiro caractere em diante (se houver), ele pode ser letra, dígito ou os 
símbolos `"."` e `"_"` 
...
1. Funções logarítmicas `log()`, `log10()` e `log2()`
